...
async function runTests(): Promise<void> {
  ...
  await test('1. ConfidenceCalculator: single source no gaps floors at 0.4', () => {
    const score = ConfidenceCalculator.compute(1, []);
    assertEqual(score, 0.4, 'single source no gaps = max(0.3, 0.4)');
  });
  ...
  await test('7. ConfidenceCalculator: floor at 0', () => {
    ...
    // (0.3 - 0.45) * 0.7 → negative → clamped to 0
  ...
  await test('9. ConfidenceCalculator.fromToolCall: success without output', () => {
    ...
    // sources=1 (success) + gap(empty_output reducing)
    // 0.3 - 0.15 = 0.15, * 0.7 (softer single-source penalty) = 0.105 → rounds to 0.11
